...
class WheelPicker @JvmOverloads constructor(
    ...
    private var mSelectedTextScale = 0.5f
    private var mTypefaceIndex: Int = 0
    val typefaceUnselected = ResourcesCompat.getFont(context, R.font.regular)
    val typefaceSelected = ResourcesCompat.getFont(context, R.font.semi_bold)
    ...
    init {
        ...
        mSelectedTextScale =
            attributesArray.getFloat(R.styleable.WheelPicker_selectedTextScale, 0.5f)
        ...
        mUnSelectedTextColor = attributesArray.getColor(
            ...
            ContextCompat.getColor(context, R.color.color_9D9696)
        )
        mTextSize = attributesArray.getDimensionPixelSize(
            R.styleable.WheelPicker_timeTextSize,
            DEFAULT_TEXT_SIZE
        )
        ...
        while (i < mSelectorItemIndices.size) {
            ...
            if (mSelectorItemValidStatus[i]) {
                if (offsetToMiddle < mItemHeight / 2) {
                    ...
                    mTextPaint.typeface = typefaceSelected
                    canvas.drawLine(
                        0f,
                        (bottom - top).toFloat() / 2 - 72,
                        (right - left).toFloat(),
                        (bottom - top).toFloat() / 2 - 72,
                        Paint(mTextPaint).apply {
                            color = mUnSelectedTextColor
                            strokeWidth = 4f
                        }
                    )
                    canvas.drawLine(
                        0f,
                        (bottom - top).toFloat() / 2 + 52,
                        (right - left).toFloat(),
                        (bottom - top).toFloat() / 2 + 52,
                        Paint(mTextPaint).apply {
                            color = mUnSelectedTextColor
                    ...
                    mTextPaint.typeface = typefaceUnselected
                }
            } else {
    ...
    fun getValue(position: Int): String = when {
        mAdapter != null -> mAdapter!!.getValue(position)
            ...
            when {
                position > mMaxIndex -> ""
                position < mMinIndex -> ""
                else -> String.format("%02d", position)
            }
        } else {
            String.format("%02d", getWrappedSelectorIndex(position))
        }
    }
