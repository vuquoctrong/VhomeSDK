...
class BaseDialog {
    ...
    fun setTitleWithColor(message: String?, color: String): BaseDialog {
        builder.setTitle(Html.fromHtml("<font color='$color'>$message</font>"))
        return this
    }
    ...
    fun setPositiveButtonWithColor(
        @StringRes label: Int,
        color: String,
        context: Context?,
        ...
        builder.setPositiveButton(
            Html.fromHtml("<font color='$color'>${context?.getString(label)}</font>")
        ) { dialog: DialogInterface, _: Int ->
            dialog.cancel()
            initialize()
            onDialogListener?.onClick()
...
//    }
    ...
    fun show() {
        if (!isShown) {
            ...
            dialog.show()
            forceShown()
            val textSize = dialog.context.resources.getDimension(R.dimen.text_size_10)
