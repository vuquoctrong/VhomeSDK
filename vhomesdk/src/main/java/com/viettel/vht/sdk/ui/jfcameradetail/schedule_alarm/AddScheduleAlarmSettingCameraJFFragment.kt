...
@AndroidEntryPoint
class AddScheduleAlarmSettingCameraJFFragment :
    BaseFragment<FragmentAddScheduleAlarmCameraJfBinding, AddScheduleAlarmSettingCameraJFViewModel>() {

    @Inject
    lateinit var appNavigation: AppNavigation

    private val viewModel: AddScheduleAlarmSettingCameraJFViewModel by viewModels()

    override val layoutId = R.layout.fragment_add_schedule_alarm_camera_jf

    override fun getVM(): AddScheduleAlarmSettingCameraJFViewModel = viewModel
    ...
    private fun initData() {
        ...
        arguments.takeIf { it?.containsKey(ARG_ID_SCHEDULE_PARAM) == true }?.let {
            viewModel.scheduleId = it.getInt(ARG_ID_SCHEDULE_PARAM, -1)
        }
        viewModel.getConfig()
    }
